...
import { advanceSea, populateSea } from '../game/SeaEncounters'
import { isInBounds, isLand, isHarbor, getMapDimensions } from '../utils/MapUtils'
...
export class WorldMapScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(this.mapX, 26, 'KATTEGAT', {
      fontFamily: 'Georgia, serif', fontSize: '30px', color: '#293f3b', fontStyle: 'bold',
    })
    localizedText(this, width - this.mapX, 45, () => t('titleLocation'), {
    ...
    const data = Array.from({ length: this.mapData.dimensions.height }, (_, y) =>
      Array.from({ length: this.mapData.dimensions.width }, (_, x) => {
        if (!isLand(x, y)) return (x * 3 + y * 7) % 4
        const edges = [[0, -1], [1, 0], [0, 1], [-1, 0]]
        return 4 + edges.reduce((mask, [dx, dy], index) =>
          mask | (isInBounds(x + dx, y + dy) && !isLand(x + dx, y + dy) ? 1 << index : 0), 0)
      })
    )
    this.tilemap = this.make.tilemap({ data, tileWidth: this.tileSize, tileHeight: this.tileSize })
    const tileset = this.tilemap.addTilesetImage('chart-tiles')!
    this.tilemap.createLayer(0, tileset, this.mapX, this.mapY)!.setCollisionBetween(4, 19)

    const chartWidth = this.tilemap.widthInPixels
    const chartHeight = this.tilemap.heightInPixels
    ...
    for (const [name, x, y] of [['Jylland', 2, 1], ['Sjælland', 20, 11], ['Sverige', 28, 4]] as const) {
      this.add.text(this.mapX + x * this.tileSize, this.mapY + y * this.tileSize, name, {
        fontFamily: 'Georgia, serif', fontSize: '16px', fontStyle: 'italic', color: '#344e3d',
      }).setOrigin(0.5)
    }

    // Draw harbor markers (optional visual overlay)
    this.mapData.harborLocations.forEach((loc: { x: number; y: number; name: string }) => {
      const px = this.mapX + loc.x * this.tileSize + this.tileSize / 2
      const py = this.mapY + loc.y * this.tileSize + this.tileSize / 2
      this.add
        .circle(px, py, 6, 0x9e493a)
        .setStrokeStyle(2, 0xf3dfad)
        .setDepth(100)
      this.add.text(px + (loc.x > 22 ? -12 : 12), py, loc.name, {
        fontFamily: 'Georgia, serif', fontSize: '13px', color: '#fff0c4',
        backgroundColor: '#244b4b', padding: { x: 4, y: 2 },
      }).setOrigin(loc.x > 22 ? 1 : 0, 0.5).setDepth(100)
    })
  ...
  private handleInput(key: string) {
    ...
    // Check for harbor
    const harbor = isHarbor(newX, newY)
    if (harbor) {
    ...
    if (encounter.stormDamage) {
      ...
      this.cameras.main.flash(180, 190, 210, 220)
    }
    this.updateUI()
  ...
  private drawSeaContacts() {
    ...
    for (const contact of this.gameState.seaContacts ?? []) {
      ...
      const marker = this.add.container(px, py).setSize(30, 30).setInteractive()
      if (contact.kind === 'enemy') {
        marker.add(this.add.circle(0, 0, 14, 0x873c30, 0.7).setStrokeStyle(1, 0xffd49a))
        marker.add(this.add.image(0, 0, 'ships_enemy', `${frames[contact.enemyId - 1]}_topview`).setDisplaySize(17, 27))
        ...
        marker.add(cloud)
        this.tweens.add({ targets: marker, alpha: 0.65, duration: 1100, yoyo: true, repeat: -1 })
        const describe = () => setLocalizedText(this.seaMessage, () => t('stormSighting'))
        marker.on('pointerover', describe).on('pointerdown', describe)
      }
