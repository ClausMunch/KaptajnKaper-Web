...
function New-ShipFrame([int]$Damage, [int]$Phase) {
    ...
    $lean = @(0, -1, 0, 1)[$Phase]
    $billow = @(0, 1, 1, 0)[$Phase]

    # Hull and anchor stay fixed; the elevated rig and canvas rock about the keel.
    ...
    # Bowsprit, standing rigging and the mast shadows establish the overhead view.
    ...
    # Two broad square sails, with stepped hems and one-pixel stitched seams.
    ...
    # Tears reveal the deck/rigging colors rather than punching through the hull.
    if ($Damage -ge 1) {
        Fill-PixelPolygon sailDark @(39, 25, 42, 25, 41, 27, 43, 29, 39, 30, 38, 28)
        Fill-PixelPolygon shadow @(40, 26, 41, 27, 40, 29, 39, 28)
        Draw-PixelLine sailDark 21 41 23 42
    }
    if ($Damage -ge 2) {
        Fill-PixelPolygon sailDark @(18, 26, 21, 27, 23, 30, 23, 33, 18, 33, 17, 30)
        Fill-PixelPolygon woodDark @(18, 29, 20, 28, 21, 31, 23, 33, 19, 33)
        Fill-PixelPolygon shadow @(36, 38, 39, 38, 40, 41, 38, 43, 35, 42)
        Fill-Pixels woodDark 37 39 2 3
        Draw-PixelLine sailDark 28 24 27 28
    }
    if ($Damage -ge 3) {
        Fill-PixelPolygon char @(25, 24, 28, 25, 28, 28, 30, 29, 29, 33, 24, 33, 25, 29, 23, 27)
        Fill-PixelPolygon woodDark @(26, 26, 27, 29, 29, 31, 28, 33, 25, 33, 26, 29)
        Fill-PixelPolygon char @(19, 40, 22, 40, 25, 43, 29, 42, 30, 46, 21, 46)
        Fill-PixelPolygon woodDark @(21, 42, 24, 44, 28, 44, 29, 46, 22, 46)
        Draw-PixelLine char 45 24 44 28
        Fill-Pixels sailDark 43 29 3 2
    }
    ...
    # An oversized, readable Dannebrog identifies the player even when damaged.
    $flagY = @(0, -1, 0, 1)[$Phase]
...
Write-Output "Created ship_player.png and ship_player.json in $OutputDirectory"
